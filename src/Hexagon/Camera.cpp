...
void Camera::setup_dimentional_projection(ALLEGRO_BITMAP* bitmap)
{
   if (!(al_is_system_installed()))
   {
      ...
      error_message << "[Hexagon::Camera::setup_dimentional_projection]: error: guard \"al_is_system_installed()\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Hexagon::Camera::setup_dimentional_projection]: error: guard \"al_is_system_installed()\" not met");
   }
   if (!(bitmap))
   {
      std::stringstream error_message;
      error_message << "[Hexagon::Camera::setup_dimentional_projection]: error: guard \"bitmap\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Hexagon::Camera::setup_dimentional_projection]: error: guard \"bitmap\" not met");
   }
