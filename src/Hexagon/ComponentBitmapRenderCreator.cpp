...
ALLEGRO_BITMAP* ComponentBitmapRenderCreator::create_bitmap_render()
{
   if (!(component))
   {
      ...
      throw std::runtime_error("ComponentBitmapRenderCreator::create_bitmap_render: error: guard \"component\" not met");
   }
   if (!(al_is_system_installed()))
   {
      ...
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("ComponentBitmapRenderCreator::create_bitmap_render: error: guard \"al_is_system_installed()\" not met");
   }
   return nullptr;
}
