...
ALLEGRO_BITMAP* TextPixelPreviewCreator::create()
{
   if (!(al_is_system_installed()))
   {
      ...
      error_message << "[Hexagon::Elements::TextPixelPreviewCreator::create]: error: guard \"al_is_system_installed()\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Hexagon::Elements::TextPixelPreviewCreator::create]: error: guard \"al_is_system_installed()\" not met");
   }
   if (!((!text_lines.empty())))
   {
      std::stringstream error_message;
      error_message << "[Hexagon::Elements::TextPixelPreviewCreator::create]: error: guard \"(!text_lines.empty())\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Hexagon::Elements::TextPixelPreviewCreator::create]: error: guard \"(!text_lines.empty())\" not met");
   }
